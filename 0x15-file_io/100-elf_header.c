...
/**
 * check_elf - Check if file = ELF file
...
void print_magic(unsigned char *e_ident)
{
	...
	for (point = 0; point < EI_NIDENT; point++)
	{
		printf("%02x", e_ident[point]);

		if (point == EI_NIDENT - 1)
			printf("\n");
		else
			printf(" ");
	}
 ...
 * @e_ident: pointer to an array containing the ELF file specs
 * Return: void
 */
 ...
 * @e_ident: pointer to an array containing the ELF ABI spesification
 * Return: void
 */
 ...
 * @e_type: ELF type
 * @e_ident: pointer to an array containing the ELF type class
 * Return: void
 */
...
/**
 * print_entry - Prints the entry point of ELF header file
